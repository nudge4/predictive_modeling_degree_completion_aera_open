...
optimizer = keras.optimizers.Adam(lr=0.0005) # default learning rate value for Adam is 0.001
model.compile(loss='binary_crossentropy', optimizer=optimizer)
gen = generate_batches(X_train_part1, X_train_part2, y_train)
callbacks = [EarlyStopByAUC(y_valid, len(X_valid_part1), model_name="lstm_model_4")]
...
history_df.to_csv("output/final_learning_curve_lstm_model_4.csv", index=False)

# Estimate the performance of the best model on the held-out test set
saved_model = keras.models.load_model("output/lstm_model_4.h5")
test_auc = \
...
model.compile(loss='binary_crossentropy', optimizer=optimizer)
gen = generate_batches(X_train_part1, X_train_part2, y_train)
callbacks = [EarlyStopByAUC(y_valid, len(X_valid_part1), model_name = "lstm_model_5")]
history = model.fit_generator(gen, steps_per_epoch = len(X_train_part1), 
                              epochs=500,verbose=0,
                              validation_data=generate_batches(X_valid_part1, X_valid_part2, y_valid),
                              validation_steps=len(X_valid_part1),
...
history_df = pd.DataFrame({'train_loss': history.history["loss"],
                           'val_loss': history.history["val_loss"]}).loc[:,['train_loss', 'val_loss']]
history_df.to_csv("output/final_learning_curve_lstm_model_5.csv", index=False)

# Estimate the performance of the best model on the held-out test set
saved_model = keras.models.load_model("output/lstm_model_5.h5")
test_auc = \
roc_auc_score(y_test, np.asarray(saved_model.predict_generator(generate_test_batches(X_test_part1, X_test_part2),
                                                               steps=len(X_test_part1))))
print("\nLSTM Model 5:\nC-statistic = {}\n".format(test_auc))
...
np.random.seed(42)
tf.random.set_seed(42)
part_1 = keras.models.Sequential([keras.layers.LSTM(40, return_sequences=True, input_shape=[None, 19]),
                                  keras.layers.LSTM(20, input_shape=[None, 20])])
part_2 = keras.models.Sequential([keras.layers.Input(shape=(55,))])
model_concat = keras.layers.concatenate([part_1.output, part_2.output], axis=-1)
model_concat = keras.layers.Dense(20, activation='sigmoid')(model_concat)
...
model.compile(loss='binary_crossentropy', optimizer=optimizer)
gen = generate_batches(X_train_part1, X_train_part2, y_train)
callbacks = [EarlyStopByAUC(y_valid, len(X_valid_part1), model_name = "lstm_model_6")]
history = model.fit_generator(gen, steps_per_epoch = len(X_train_part1), 
                              epochs=500,verbose=0,
                              validation_data=generate_batches(X_valid_part1, X_valid_part2, y_valid),
                              validation_steps=len(X_valid_part1),
...
history_df = pd.DataFrame({'train_loss': history.history["loss"],
                           'val_loss': history.history["val_loss"]}).loc[:,['train_loss', 'val_loss']]
history_df.to_csv("output/final_learning_curve_lstm_model_6.csv", index=False)

# Estimate the performance of the best model on the held-out test set
saved_model = keras.models.load_model("output/lstm_model_6.h5")
test_auc = \
roc_auc_score(y_test, np.asarray(saved_model.predict_generator(generate_test_batches(X_test_part1, X_test_part2),
                                                               steps=len(X_test_part1))))
print("\nLSTM Model 6:\nC-statistic = {}\n".format(test_auc))