...
class EarlyStopByAUC(keras.callbacks.Callback):
    ...
    def __init__(self, y, nb, verbose = 1, patience = 5, delta = 1e-4, model_name='lstm_model_1_simple'):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        .to_csv("output_3/running_learning_curve_{}.csv".format(self.model_name), index=False)
        if self.verbose > 0:
            print("Epoch {0}: val_auc = {1}".format(epoch+1, score))
        if self.best_score is None:
            self.best_score = score
            self.model.save("output_3/{}.h5".format(self.model_name))
...
np.random.seed(42)
tf.random.set_seed(42)
part_1 = keras.models.Sequential([keras.layers.LSTM(20, return_sequences=True, input_shape=[None, 8]),
                                  keras.layers.LSTM(20, input_shape=[None, 20])])
part_2 = keras.models.Sequential([keras.layers.Input(shape=(59,))])
...
callbacks = [EarlyStopByAUC(y_valid, len(X_valid_part1), model_name="lstm_model_simple")]
...
history_df.to_csv("output_3/final_learning_curve_lstm_model_simple.csv", index=False)

# Estimate the performance of the best model on the held-out test set
saved_model = keras.models.load_model("output_3/lstm_model_simple.h5")
test_auc = \
