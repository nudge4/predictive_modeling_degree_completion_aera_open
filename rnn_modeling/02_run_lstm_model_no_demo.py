...
X_train_part1 = pickle.load(open("lstm_data_2/part1_train_X.p", "rb"))
X_valid_part1 = pickle.load(open("lstm_data_2/part1_valid_X.p", "rb"))
X_test_part1 = pickle.load(open("lstm_data_2/part1_test_X.p", "rb"))
X_train_part2 = np.load("lstm_data_2/part2_train_X.npy")
X_valid_part2 = np.load("lstm_data_2/part2_valid_X.npy")
X_test_part2 = np.load("lstm_data_2/part2_test_X.npy")
y_train = np.load("lstm_data_2/train_y.npy")
y_valid = np.load("lstm_data_2/valid_y.npy")
y_test = np.load("lstm_data_2/test_y.npy")


class EarlyStopByAUC(keras.callbacks.Callback):
    
    def __init__(self, y, nb, verbose = 1, patience = 5, delta = 1e-4, model_name='lstm_model_1_demo'):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        .to_csv("output_2/running_learning_curve_{}.csv".format(self.model_name), index=False)
        if self.verbose > 0:
            print("Epoch {0}: val_auc = {1}".format(epoch+1, score))
        if self.best_score is None:
            self.best_score = score
            self.model.save("output_2/{}.h5".format(self.model_name))
...
#########################################################################################################################
#Fit Model: Use the optimal model hyperparameter/architecture identified from the base RNN model, for simplicity purposes
#########################################################################################################################
print("\n\n\n")
print("Fitting Model 1:")
np.random.seed(42)
tf.random.set_seed(42)
part_1 = keras.models.Sequential([keras.layers.LSTM(20, return_sequences=True, input_shape=[None, 17]),
                                  keras.layers.LSTM(20, input_shape=[None, 20])])
part_2 = keras.models.Sequential([keras.layers.Input(shape=(40,))])
...
callbacks = [EarlyStopByAUC(y_valid, len(X_valid_part1), model_name="lstm_model_no_demo")]
...
history_df.to_csv("output_2/final_learning_curve_lstm_model_no_demo.csv", index=False)

# Estimate the performance of the best model on the held-out test set
saved_model = keras.models.load_model("output_2/lstm_model_no_demo.h5")
test_auc = \
