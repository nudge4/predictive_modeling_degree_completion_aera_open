...
// (Since when we exclude all NSC data, the graduation time might change according to our definition, it's necessary to repeat the procedure of random truncation as we did for the base models) 
...
merge 1:1 vccsid using "${main_dir}/dta/student_level_sample_and_outcomes.dta", keep(1 3) keepusing(deg_vccs_*_strm) nogen
egen first_degree_strm_vccs = rowmin(deg_vccs_*_strm)
rename first_degree_strm_vccs first_degree_strm
...
save "${main_dir}/intermediate_files/enrolled_nth_alternative.dta", replace
