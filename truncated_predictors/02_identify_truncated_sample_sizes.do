...
qui count if first_degree_strm < 2124
di r(N)/_N
count if first_degree_strm == 2124
di r(N)/_N
drop if first_degree_strm <= 2124
...
gen train_sample_size = round(prop * 298139) // This number could be found using the files "full_data_enrolled_terms.dta" and "enrolled_nth.dta"
egen total_sample_size = sum(train_sample_size)
replace train_sample_size = train_sample_size + (298139 - total_sample_size) if _n == 1
drop total*
gen valid_sample_size = round(prop * 33115) // This number could be found using the files "full_data_enrolled_terms.dta" and "enrolled_nth.dta"
egen total_sample_size = sum(valid_sample_size)
replace valid_sample_size = valid_sample_size + (33115 - total_sample_size) if _n == 1
drop total*

sort nth_term
save "${main_dir}/intermediate_files/truncation_sample_sizes.dta", replace
