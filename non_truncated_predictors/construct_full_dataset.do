...
* drop if strm > 2132 // (MODIFY HERE!!) discard the observations which come after the end of the one-year window
...
preserve
	use "/Users/${username}/Box Sync/VCCS public data/VCCS_Course_List/VCCS_repeatable_courses.dta", clear // list of repeatable VCCS courses -- can be found from the course descriptions on this webpage: https://courses.vccs.edu/
...
drop if strm < 2073 | strm > 2123 // we need to grab age_entry
...
reshape wide enrolled-pell_1, i(vccsid) j(suffix) string
sort vccsid
...
sort rn
gen obs_id = _n
gen valid = (obs_id <= 33350) // Randomly select the students to be included in the validation set
preserve
	keep vccsid valid first_nonde_strm enrolled_*
	save "${main_dir}/intermediate_files/full_data_enrolled_terms.dta", replace // All of the terms in which each student was actively enrolled in VCCS
restore
...
sort vccsid
save "${main_dir}/intermediate_files/full_data_6yr.dta", replace


egen num_vccs = rowtotal(product_*)
gen pct_vccs = num_vccs/available
drop num_vccs
foreach v in degree_seeking term_cred_att term_gpa prop_comp withdrawn_prop_comp lvl2_prop_comp dev_prop_comp repeat pell_0 pell_1 enrolled_nsc enrl_intensity_nsc enrolled {
...
save "C:\Users\ys8mz\Box Sync\Predictive Models of College Completion (VCCS)\intermediate_files\full_data_6yrs_no_term_specific.dta", replace
